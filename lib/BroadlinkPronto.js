...
function decodeBroadlinkIr(data) {
  ...
  const durationRange = durationsUs.reduce((range, duration) => ({
    min: Math.min(range.min, duration),
    max: Math.max(range.max, duration),
  ...
  return {
    ...
    hasTerminator,
    longGapCount,
    minDurationUs: durationRange.min,
    maxDurationUs: durationRange.max,
    packetOffset,
    storedLength,
  };
}
...
function broadlinkToPronto(data, options = {}) {
  ...
  return {
    ...
    pulseDataLength: decoded.dataLength,
    hasTerminator: decoded.hasTerminator,
    minDurationUs: Math.round(decoded.minDurationUs),
    maxDurationUs: Math.round(decoded.maxDurationUs),
