...
const BroadlinkSP4 = require("./BroadlinkSP4.js");
...
class Communicate {
  ...
  async send_packet(command, version, payload) {
    ...
    if (command !== 0x65 && response && response.data) {
    ...
    if (response) {
      ...
      if (response.error == 0) {
        ...
        response.decryptedPayload = r; // Complete frame for SP4/SP4B; preserve cmd/payload below.
        response.cmd = r.slice(0, 4); // get first 4 bytes in array
        response.payload = r.slice(0x04); // remove first 4 bytes of array

        if (command !== 0x65) this._utils.debugLog(
          this,
          "<-send_packet: payload =" +
            this.ipAddress +
            ": cmd :" +
  ...
  async auth() {
    ...
    this._utils.debugLog(this, "<== auth: authentication succeeded");

    return authData;
  }

  async scb1e_get_state() {
    const variant = BroadlinkSP4.getVariant(this.deviceType);
    const response = await this.send_packet(0x6a, false, BroadlinkSP4.encode(variant, 1, {}));
    return BroadlinkSP4.decode(variant, response, true);
  }

  async scb1e_set_power(value) {
    if (typeof value !== "boolean") throw new Error("SCB1E relay state must be a boolean");
    const variant = BroadlinkSP4.getVariant(this.deviceType);
    const response = await this.send_packet(0x6a, false, BroadlinkSP4.encode(variant, 2, { pwr: Number(value) }));
    return BroadlinkSP4.decode(variant, response);
  }
   ...
   *
   */
  async discover(timeout, local_ip_address, device_ip_address, localPort = 44488) {
    ...
    var port = localPort; // Zero uses the reply address/port, as in upstream hello().
    ...
    if (!response.data || response.data.length < 0x40) {
      throw new Error("Incomplete Broadlink discovery response");
    }
    let info = {
      ipAddress: response.address,
      mac: response.data.slice(0x3a, 0x3a + 6),
      devtype: response.data[0x34] | (response.data[0x35] << 8),
      // Older discovery frames omit this field. Never infer unlocked from absence.
      isLocked: response.data.length > 0x7f && [0, 1].includes(response.data[0x7f])
        ? response.data[0x7f] === 1 : null,
    };
  ...
  async checkTempHumidity_rm4pro() {
    ...
    try {
      ...
      if (!resp || resp.length < 6) {
        throw new Error("Incomplete RM4 temperature/humidity response");
      }
      // send_packet strips four header bytes; RM4 has a six-byte header.
      // Temperature bytes are signed, humidity bytes are unsigned (hundredths).
      let temperature = [resp[0x2] << 24 >> 24, resp[0x3] << 24 >> 24];
      let humidity = [resp[0x4], resp[0x5]];
      if (resp.slice(2, 6).every((value) => value === 0)) {
        this._utils.debugLog(this, "RM4 returned all-zero sensor values. Check the HTS2 sensor cable connection and compare readings in the Broadlink app; this response does not identify the cause.");
      }
  ...
  encodeRFFrequency_rm4pro(frequencyMHz) {
    if (frequencyMHz === undefined || frequencyMHz === null || frequencyMHz === 0) return null;
    if (!Number.isFinite(frequencyMHz)
      || !((frequencyMHz >= 305 && frequencyMHz <= 335) || (frequencyMHz >= 430 && frequencyMHz <= 440))) {
      throw new Error("RF frequency must be 305–335 or 430–440 MHz; use 0 for automatic scanning. The band must be supported by your RM4 Pro.");
    }
    const bytes = Buffer.alloc(4);
    bytes.writeUInt32LE(Math.round(frequencyMHz * 1000));
    return bytes;
  }
  ...
  async checkRFData2_rm4pro(frequencyBytes) {
    ...
    // Four command bytes plus four frequency bytes (MHz * 1000, little endian).
    payload.set([0x08, 0x00, 0x1b], 0);
    payload.set(frequencyBytes, 6);
    await this._check_data(payload);

    const readPayload = new Uint8Array([0x04, 0x00, 0x04, 0x00, 0x00, 0x00]);

    let retryCount = 10;

    do {
        this._utils.debugLog(this, `>>> Sending packet to check captured RF data, retry: ${10 - retryCount} <<<`);
        let response = await this._check_data(readPayload);
        this._utils.debugLog(
            this,
